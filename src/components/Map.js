...
function Map({
  ...
  function loadEpoly() {
    ...
    window.google.maps.Polygon.prototype.GetPointAtDistance = function (metres) {
      ...
      let dist = 0
      let olddist = 0
      for (let i = 1; (i < this.getPath().getLength() && dist < metres); i + 1) {
        olddist = dist
      ...
      return new window.google.maps.LatLng(p1.lat() + (p2.lat()
      - p1.lat()) * m, p1.lng() + (p2.lng() - p1.lng()) * m)
    }
    ...
    window.google.maps.Polygon.prototype.GetPointsAtDistance = function (metres) {
      ...
      let dist = 0
      let olddist = 0
      for (let i = 1; (i < this.getPath().getLength()); i + 1) {
        olddist = dist
        dist += this.getPath().getAt(i).distanceFrom(this.getPath().getAt(i - 1))
        while (dist > next) {
          ...
          points.push(new window.google.maps.LatLng(p1.lat() + (p2.lat() - p1.lat())
          * m, p1.lng() + (p2.lng() - p1.lng()) * m))
          next += metres
        }
      }
      return points
